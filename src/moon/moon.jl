...
"""
    moon_position_mod(jd_tdb::Number[, model])
    moon_position_mod(date_tdb::DateTime[, model])
...
[2, p. 337] that provides an accuracy of 10" in the longitude and 4" in the latitude (the
reference does not mention the timespan). `Val(:Vallado)` uses the algorithm in [1, p. 288]
that is 10x faster than `Val(:Meeus)` but can lead to errors of 0.3° in longitude and 0.2°
in latitude.
